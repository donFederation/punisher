...
# set the MULTIPUNISH constant to stop the script automatically loading theme config files.
if (!defined('MULTIPUNISH') && file_exists($tmp = PUNISH_ROOT . '/themes/' . $CONFIG['theme'] . '/config.php')) {
...
function getTemplatePath($file)
{
    ...
    if (!file_exists($return = PUNISH_ROOT . '/themes/' . $CONFIG['theme'] . '/' . $file . '.php')) {

        # Then look in default folder (if different)
        if ($CONFIG['theme'] == 'default' || !file_exists($return = PUNISH_ROOT . '/themes/default/' . $file . '.php')) {
